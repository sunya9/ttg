...
module.exports = (req, res) => {
  const { consumer_key, consumer_secret, type } = req.query
  const invalidParams = !consumer_key || !consumer_secret || !type
  if(invalidParams) {
    new Error(400, 'Invalid parameters.')
  }
  ...
  new Promise((resolve, reject) => {
    ...
    const { oauthToken, oauthTokenSecret, url } = pres
  ...
  }).catch(e => new Error(400, JSON.stringify(e)))
}